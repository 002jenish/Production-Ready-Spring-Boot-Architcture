...
import { generateBuildGradle, generateSettingsGradle } from "./gradle-generator";
import { generateApplicationYml, generateApplicationDevYml, generateApplicationProdYml, getConfigFilenames } from "./yml-generator";
import {
...
const GRADLEW_CONTENT = `#!/bin/sh
# Gradle Wrapper script
exec gradle "$@"
`;

const GRADLEW_BAT_CONTENT = `@echo off
...
export async function generateZip(req: GenerateRequest): Promise<Buffer> {
  ...
  // ── Root & Build Tool files ───────────────────────────────────────────────
  if (req.buildTool === "gradle") {
    zip.file("build.gradle", generateBuildGradle(req));
    zip.file("settings.gradle", generateSettingsGradle(req));
    zip.file("gradlew", GRADLEW_CONTENT);
    zip.file("gradlew.bat", GRADLEW_BAT_CONTENT);
  ...
  zip.file(`${resources}/${cfgFiles.main}`, generateApplicationYml(req));
...
function generateReadme(req: GenerateRequest): string {
  ...
  const runCmd = isGradle ? "./gradlew bootRun --args='--spring.profiles.active=dev'" : "./mvnw spring-boot:run -Dspring-boot.run.profiles=dev";
  const buildCmd = isGradle ? "./gradlew build -x test\njava -jar build/libs/" + req.artifactId + "-0.0.1-SNAPSHOT.jar" : "./mvnw clean package -DskipTests\njava -jar target/" + req.artifactId + "-0.0.1-SNAPSHOT.jar";
  const buildToolName = isGradle ? "Gradle 8.5+" : "Maven 3.9+";
...
- **Build Tool**: ${isGradle ? "Gradle" : "Maven"}
